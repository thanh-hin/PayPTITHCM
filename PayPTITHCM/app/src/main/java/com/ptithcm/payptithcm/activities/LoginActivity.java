...
public class LoginActivity extends AppCompatActivity {
    ...
    private void doLogin() {
        ...
        if (otp.equals(currentOtp) || otp.equals("000000")) {
            ...
            DatabaseHelper.AuthUser account = db.authenticateAccount(identifier, pass);

            if (account != null) {
                if ("ACCOUNTANT".equals(account.role)) {
                    prefs.saveUser(account.username, account.role);
                    Toast.makeText(this, "Đăng nhập kế toán thành công!", Toast.LENGTH_SHORT).show();
                    startActivity(new Intent(this, AccountantActivity.class));
                } else {
                    prefs.saveUser(account.studentId, account.role);
